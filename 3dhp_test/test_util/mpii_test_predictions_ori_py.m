...
load(['../../checkpoint/model_3dhp_ddhpose_featuremean_0809/inference_data_' aggregation_mode '.mat']) %Change to wherever you put the predicted 3D poses.
t_size = size(TS1);
for tt = 1:1
    ...
    %%
        ...
        for j = 1:length(dat.valid_frame)
            if(dat.valid_frame(j))
                ...
                pred_p = pred_seq(:,:,j); %Replace with the actual prediction formatted as 3x17; 
                error_p = (pred_p - P).^2;
    ...
    save(['.' '/model_3dhp_ddhpose_featuremean_0809' filesep 'mpii_3dhp_prediction_ori_' aggregation_mode '_t' num2str(tt) '.mat'], 'sequencewise_per_joint_error', 'sequencewise_activity_labels');
    [seq_table, activity_table] = mpii_evaluate_errors(sequencewise_per_joint_error, sequencewise_activity_labels);

    out_file = ['./' 'model_3dhp_ddhpose_featuremean_0809' '/H1_K1' filesep 'mpii_3dhp_evaluation'];
